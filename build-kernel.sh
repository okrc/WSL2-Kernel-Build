...
if [ ! -f 'kernel.tar.xz' ]; then
    KERNEL_URL=https://api.github.com/repos/microsoft/WSL2-Linux-Kernel/tarball/linux-msft-wsl-6.6.36.3
    curl -C- -sL ${KERNEL_URL} -o kernel.tar.xz
fi

TMPDIR=$(mktemp -d)
tar xf kernel.tar.xz --strip-components=1 -C $TMPDIR
curl --silent --remote-name-all https://raw.githubusercontent.com/microsoft/WSL2-Linux-Kernel/linux-msft-wsl-6.6.y/arch/{x86/configs/config-wsl,arm64/configs/config-wsl-arm64}
